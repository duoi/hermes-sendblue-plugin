    ...
    if os.path.exists(txt_path):
        with open(txt_path, "r") as f:
            for line in f:
                ...
                if handle:
                    try:
                        conn.execute("INSERT INTO processed_messages (message_handle, status) VALUES (?, 'completed')", (handle,))
...
def mark_processing(handle, number):
    conn = sqlite3.connect(DB_PATH)
    try:
        conn.execute("INSERT INTO processed_messages (message_handle, sender_number) VALUES (?, ?)", (handle, number))
        conn.commit()
        return True
    except sqlite3.IntegrityError:
        return False
...
def update_status(handle, status, error=None):
    conn = sqlite3.connect(DB_PATH)
    conn.execute("UPDATE processed_messages SET status = ?, error_log = ? WHERE message_handle = ?", (status, error, handle))
    conn.commit()
    conn.close()
    ...
    def __init__(self):
        ...
        if self.bucket and aws_key and aws_secret:
            try:
                ...
                print("⚠️  WARNING: S3 credentials found, but boto3 is not installed. Falling back to tmpfiles.org!")
        
        if not self.use_s3:
            print("⚠️  WARNING: No S3/R2 credentials found. Outbound media will be uploaded to PUBLIC tmpfiles.org!")
            ...
            if os.path.exists(media_path):
                if media_path.endswith('.ogg') or media_path.endswith('.mp3'):
                    ...
                    if shutil.which("ffmpeg"):
                        proc = await asyncio.create_subprocess_exec("ffmpeg", "-y", "-i", media_path, "-acodec", "libopus", "-b:a", "32k", "-vbr", "on", "-compression_level", "10", new_path, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
...
def send_typing_indicator_sync():
    ...
    headers = {"sb-api-key-id": API_KEY, "sb-api-secret-key": API_SECRET, "Content-Type": "application/json"}
    try:
        requests.post(url, headers=headers, json={"number": USER_PHONE, "from_number": SENDBLUE_PHONE}, timeout=3)
    except:
        pass
...
async def process_message(msg):
    ...
    try:
        ...
        if not content and media_url:
            print(f"--> Received media from {media_url}. Delegating natively to Hermes...")
            async with aiohttp.ClientSession() as session:
                async with session.get(media_url) as resp:
                    if resp.status == 200:
                        ext = media_url.split(".")[-1] if "." in media_url[-5:] else "caf"
                        fd, temp_path = tempfile.mkstemp(suffix=f".{ext}")
                        with os.fdopen(fd, 'wb') as f:
                            f.write(await resp.read())
                        content = f"MEDIA: {temp_path}"
                    else:
                        content = f"[Failed to download inbound media: HTTP {resp.status}]"

        if not content:
            update_status(handle, "completed", "empty content")
            return

        prefix_enabled = os.environ.get("SENDBLUE_PREFIX_ENABLED", "true").lower() == "true"
...
[via SendBlue] """
        ...
        if content.lower() in ["/new", "reset"]:
            ...
            cmd = [get_hermes_bin(), "chat", "-Q", "-q", "Hello! This is a brand new session. What's up?"]
            ...
            async with session.get(url, headers=headers) as resp:
                if resp.status == 200:
                    ...
                    for msg in data.get("data", []):
                        ...
                        if h: mark_processing(h, msg.get("from_number"))
        except Exception as e:
