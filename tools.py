...
def check_admin_access(task_id: str) -> bool:
    ...
    if not admins:
        logger.warning(
            "No SENDBLUE_ADMIN_PHONES or USER_PHONE configured. Rejecting tool access."
        )
        return False
...
def sendblue_send_message(args: Dict[str, Any], task_id: str = None, **kwargs) -> str:
    if not check_admin_access(task_id):
        return json.dumps(
            {
                "error": "Unauthorized: Sendblue tools are restricted to admin phone numbers."
            }
        )
    ...
    if not all([api_key, api_secret, from_num]):
        return json.dumps(
            {
                "error": "Sendblue API keys (SENDBLUE_API_KEY, SENDBLUE_API_SECRET, SENDBLUE_PHONE) not configured in environment."
            }
        )
    ...
    try:
        ...
        resp = requests.post(
            url,
            headers=headers,
            json={"number": number, "content": message, "from_number": from_num},
        )
        ...
        schema={
            ...
            "parameters": {
                ...
                "properties": {
                    "number": {
                        "type": "string",
                        "description": "The destination E.164 phone number (e.g. +14155551234)",
                    },
                    "message": {
                        "type": "string",
